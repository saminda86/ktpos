...
// Description: Professional Invoice View (Quotation Style Design + S/N & Image)
...
// 4. දත්ත ලබා ගැනීම (Invoice + Client + User)
...
// 5. Items, Description, Image සහ Serial Number ලබා ගැනීම
$items = [];
// Joining with products table to get image and description
...
<head>
    ...
    <style>
        ...
        /* --- SMART ACTION BAR (Same as Quotation) --- */
        .action-bar {
            position: fixed;
            top: 20px;
            left: 50%;
        ...
        .item-sn { font-size: 0.75rem; color: #555; font-weight: 600; margin-top: 2px; }
...
<body>
    ...
    <div class="action-bar no-print" id="floatingBar">
        ...
        <a href="view_invoice.php?id=<?php echo $invoice_id; ?>&orientation=<?php echo $is_landscape ? 'portrait' : 'landscape'; ?>" 
           class="btn btn-glass-light" title="Rotate Page">
            <i class="fas fa-sync-alt"></i> <span><?php echo $is_landscape ? 'Portrait' : 'Landscape'; ?></span>
        </a>
    ...
    <div class="page-container" id="captureArea">
        ...
        <header class="header-section">
            ...
            <div class="document-title">
                <h1>INVOICE</h1>
                <span class="real-title">TAX INVOICE</span>
            </div>
        </header>
        ...
        <div class="info-grid">
            <div class="info-box">
                ...
                <div class="client-details">
                    <div class="client-detail-row"><?php echo nl2br(htmlspecialchars($invoice['client_address'])); ?></div>
                    
                    <?php if($invoice['client_phone']): ?>
                        <div class="client-detail-row"><i class="fas fa-phone-alt text-muted"></i> <?php echo htmlspecialchars($invoice['client_phone']); ?></div>
                    <?php endif; ?>

                    <?php if($invoice['client_email']): ?>
                        <div class="client-detail-row"><i class="fas fa-envelope text-muted"></i> <?php echo htmlspecialchars($invoice['client_email']); ?></div>
                    <?php elseif(!empty($invoice['client_whatsapp'])): ?>
                        <div class="client-detail-row"><i class="fab fa-whatsapp text-success"></i> <?php echo htmlspecialchars($invoice['client_whatsapp']); ?></div>
                    <?php else: ?>
            ...
            <div class="info-box">
                ...
                <table class="meta-table">
                    <tr><td>Invoice #:</td><td><?php echo htmlspecialchars($invoice['invoice_number']); ?></td></tr>
                    <tr><td>Date Issued:</td><td><?php echo date('M d, Y', strtotime($invoice['invoice_date'])); ?></td></tr>
                    <tr><td>Payment Status:</td><td><span class="<?php echo getStatusColor($invoice['payment_status']); ?> fw-bold"><?php echo htmlspecialchars($invoice['payment_status']); ?></span></td></tr>
                    <tr><td>Issued By:</td><td><?php echo htmlspecialchars($invoice['user_name']); ?></td></tr>
                </table>
            </div>
        </div>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php if(!empty($item['serial_number'])): ?>
                            <div class="item-sn"><i class="fas fa-barcode me-1"></i> S/N: <?php echo htmlspecialchars($item['serial_number']); ?></div>
                        <?php endif; ?>
                    </td>
        ...
        <div class="totals-wrapper">
            <table class="totals-table">
                ...
                <tr class="grand-total-row"><td class="label">Grand Total:</td><td class="value"><span class="currency-code">LKR</span><?php echo number_format($invoice['grand_total'], 2); ?></td></tr>
            </table>
        </div>
        ...
        <div class="terms-wrapper">
            <div class="terms-box">
                ...
                <p><?php echo htmlspecialchars($invoice['invoice_terms']); ?></p>
            </div>
        </div>
        <?php endif; ?>
    ...
    <script>
        ...
        function saveAsImage(btn) {
            ...
            html2canvas(element, {
                ...
                link.download = 'Invoice_<?php echo $invoice['invoice_number']; ?>.jpg';
