...
// Description: Invoice Manager - Single File Solution (With User & Timestamp Info)
...
// =========================================================
// 1. DELETE LOGIC (HANDLED INTERNALLY)
// =========================================================
if (isset($_GET['delete_id']) && is_numeric($_GET['delete_id'])) {
    $del_id = intval($_GET['delete_id']);
    
    // 1. Restore Stock
    $sql_items = "SELECT product_id, quantity FROM invoice_items WHERE invoice_id = $del_id";
    ...
    // 2. Delete Items & Invoice Record
    $conn->query("DELETE FROM invoice_items WHERE invoice_id = $del_id");
    
    if ($conn->query("DELETE FROM invoices WHERE invoice_id = $del_id") === TRUE) {
        $msg = "ඉන්වොයිසිය සාර්ථකව මකා දමන ලදී! අලෙවි වූ භාණ්ඩ නැවත ස්ටොක් (Stock) වෙත එකතු විය.";
        header("Location: invoices.php?msg=" . urlencode($msg));
    } else {
        $error = "ඉන්වොයිසිය මැකීමේ දෝෂයක්: " . $conn->error;
...
// =========================================================
// 3. DYNAMIC DATA STATISTICS
// =========================================================
$stats = ['total_count' => 0, 'total_value' => 0, 'paid_value' => 0, 'due_value' => 0];
...
if($res_stats) { 
    while ($row = $res_stats->fetch_assoc()) { 
        ...
        if ($row['payment_status'] === 'Paid') { $stats['paid_value'] += $row['value']; } 
        else { $stats['due_value'] += $row['value']; }
    } 
}

// =========================================================
// 4. FETCH DATA (UPDATED JOIN FOR USER INFO)
// =========================================================
$limit = 10; 
...
if (!empty($search)) { 
    $where .= " AND (i.invoice_number LIKE '%$search%' OR c.client_name LIKE '%$search%' OR u.name LIKE '%$search%' OR i.invoice_id IN (SELECT invoice_id FROM invoice_items WHERE item_name LIKE '%$search%'))"; 
}

// Updated Query to fetch User Name
$sql = "SELECT i.*, c.client_name, u.name AS created_by_name,
...
$total_records = $conn->query("SELECT COUNT(*) as count FROM invoices i LEFT JOIN clients c ON i.client_id = c.client_id LEFT JOIN users u ON i.user_id = u.user_id $where")->fetch_assoc()['count'];
$total_pages = ceil($total_records / $limit);

// =========================================================
// 5. FETCH DATA FOR MODAL
// =========================================================
$clients = [];
...
$sql_p = "SELECT p.product_id, p.product_name, p.product_code, p.sell_price, p.buy_price, p.stock_quantity, p.image_path, s.supplier_name 
          FROM products p LEFT JOIN suppliers s ON p.supplier_id = s.supplier_id ORDER BY p.product_name ASC";
...
<style>
    ...
    .stat-card { background: #fff; border-radius: 12px; padding: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.02); border: 1px solid rgba(0,0,0,0.05); display: flex; align-items: center; transition: 0.2s; height: 100%; }
    ...
    .select2-container--bootstrap-5 .select2-dropdown { z-index: 9999; border-radius: 8px; box-shadow: 0 5px 20px rgba(0,0,0,0.1); border: 1px solid #e0e0e0; }
    .select2-results__option { padding: 8px 12px !important; border-bottom: 1px solid #fcfcfc; }
    .select2-container--bootstrap-5 .select2-results__option--highlighted[aria-selected] { background-color: #f3f4f6 !important; color: #212529 !important; }
    ...
    .user-name { font-size: 0.85rem; font-weight: 700; color: #344767; display: flex; align-items: center; gap: 6px; }
    .user-time { font-size: 0.7rem; color: #8392ab; margin-top: 2px; display: block; }
    .user-avatar-xs { width: 22px; height: 22px; background: #e9ecef; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: #344767; font-size: 0.7rem; }
</style>
...
<div class="container-fluid py-4">
    ...
    <div class="row g-3 mb-4">
        <div class="col-xl-3 col-sm-6"><div class="stat-card"><div class="icon-shape bg-gradient-primary shadow"><i class="fas fa-file-invoice-dollar"></i></div><div class="ms-3"><h6>Total Invoices</h6><h4><?php echo number_format($stats['total_count']); ?></h4></div></div></div>
        <div class="col-xl-3 col-sm-6"><div class="stat-card"><div class="icon-shape bg-gradient-info shadow"><i class="fas fa-coins"></i></div><div class="ms-3"><h6>Total Value</h6><h4>Rs. <?php echo number_format($stats['total_value'], 2); ?></h4></div></div></div>
        <div class="col-xl-3 col-sm-6"><div class="stat-card"><div class="icon-shape bg-gradient-success shadow"><i class="fas fa-check-double"></i></div><div class="ms-3"><h6>Paid Value</h6><h4>Rs. <?php echo number_format($stats['paid_value'], 2); ?></h4></div></div></div>
        <div class="col-xl-3 col-sm-6"><div class="stat-card"><div class="icon-shape bg-gradient-danger shadow"><i class="fas fa-exclamation-circle"></i></div><div class="ms-3"><h6>Due Value</h6><h4>Rs. <?php echo number_format($stats['due_value'], 2); ?></h4></div></div></div>
    </div>
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead>
                        <tr>
                            ...
                            <th>Issued Info</th> <th>Customer</th>
                    ...
                    <tbody>
                        <?php if ($result && $result->num_rows > 0): while ($row = $result->fetch_assoc()): 
                            ...
                            // User & Date Logic
                            $userName = !empty($row['created_by_name']) ? $row['created_by_name'] : 'System/Deleted';
                            $userInitial = strtoupper(substr($userName, 0, 1));
                            $createDate = date('d M Y', strtotime($row['invoice_date']));
                            // Using invoice_date as time base, assuming created_at might be missing in simple setup
                            // If you have a created_at column in DB, replace $row['invoice_date'] with $row['created_at'] for time
                            $createTime = isset($row['created_at']) ? date('h:i A', strtotime($row['created_at'])) : date('h:i A'); 
                            
                            // Last Update Logic (Assuming updated_at column exists, else fallback)
                            $updateTitle = isset($row['updated_at']) && $row['updated_at'] 
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="user-info-cell" data-bs-toggle="tooltip" data-bs-placement="top" title="<?php echo $updateTitle; ?>">
                                    <div class="user-name">
                                        <div class="user-avatar-xs"><?php echo $userInitial; ?></div>
                                        <?php echo htmlspecialchars($userName); ?>
                                    </div>
                                    <span class="user-time"><i class="far fa-clock me-1"></i> <?php echo $createDate; ?> at <?php echo $createTime; ?></span>
                                </div>
                            </td>
                            ...
                            <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                                <table class="table table-sm align-middle mb-0" id="iTable">
                                    <thead class="bg-white text-uppercase text-secondary text-xs">
                                        <tr><th width="3%" class="text-center">#</th><th width="5%" class="text-center">Img</th><th width="25%" style="min-width: 180px;">Product / Service</th><th width="15%" class="text-center">Serial No / Warranty</th><?php if($userType === 'Admin'): ?><th width="8%" class="text-end">Cost</th><?php endif; ?><th width="10%" class="text-end">Price</th><th width="8%" class="text-center">Qty</th><?php if($userType === 'Admin'): ?><th width="10%" class="text-end text-success">Profit</th><?php endif; ?><th width="10%" class="text-end">Total</th><th width="6%"></th></tr>
                                    </thead>
...
<div class="modal fade" id="serialModal" tabindex="-1" data-bs-backdrop="static">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            <div class="modal-header bg-primary text-white py-2"><h6 class="modal-title fw-bold small text-uppercase"><i class="fas fa-barcode me-2"></i> Enter Serial Numbers & Warranty</h6><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
            <div class="modal-body bg-white p-3">
                ...
                <div class="d-flex justify-content-between mt-2"><small class="text-muted fw-bold">Lines: <span id="serialLineCount">0</span></small><button type="button" class="btn btn-sm btn-outline-secondary py-0" onclick="$('#serialModalInput').val('').trigger('input')">Clear</button></div>
            </div>
            <div class="modal-footer bg-light p-2"><button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Close</button><button type="button" class="btn btn-primary btn-sm px-4" onclick="saveSerialFromModal()">Save Changes</button></div>
        </div>
    </div>
</div>
...
// TOAST INIT (Updated: Position Top Center)
...
$(document).ready(function() {
    // 1. Tooltip Init (For User Update Info)
    ...
    if (msg) {
        Toast.fire({ icon: 'success', title: 'සාර්ථකයි!', text: msg, customClass: { title: 'text-success fw-bold' } });
        window.history.replaceState(null, null, window.location.pathname);
    }
    if (error) {
        Swal.fire({ icon: 'error', title: 'දෝෂයක්!', text: error, confirmButtonText: 'හරි' })
    ...
    $('#invoiceForm #client_id').html(clientOpts).select2({ dropdownParent: $('#invoiceModal'), theme: 'bootstrap-5' });
    $(document).on('select2:open', () => { document.querySelector('.select2-search__field').focus(); });
    
    // 5. Initial Row
    if ($('#invoiceFormAction').val() === 'create' && $('#iTableBody tr').length === 0) { addNewInvoiceRow(); }

    // 6. Serial Modal Count
...
function formatProduct(state) {
    ...
    return $(`<div class="s2-item-container"><img src="${img}" class="s2-img"><div class="s2-details"><div class="s2-name">${p.product_name}</div><div class="s2-row ${stockClass}"><i class="fas fa-box-open" style="font-size:0.7em"></i> ${stockInfo} &nbsp;|&nbsp; <i class="fas fa-truck" style="font-size:0.7em"></i> ${sup}</div></div><div class="s2-price-box"><span class="s2-price">Rs. ${parseFloat(p.sell_price).toLocaleString('en-US', {minimumFractionDigits: 2})}</span></div></div>`);
}
...
window.loadEditInvoice = function(element) {
    ...
    $.ajax({
        url: 'invoice_load.php', type: 'GET', data: { id: invoiceId }, dataType: 'json',
        success: function(res) {
            if(res.status === 'success') {
                const invoice = res.data.invoice;
                const items = res.data.items;
                openCreateInvoiceModal(); 
                $('#invoiceFormAction').val('update'); $('#invoiceId').val(invoice.invoice_id);
                ...
                $('#invoice_date').val(invoice.invoice_date); $('#payment_status').val(invoice.payment_status);
                $('#invoice_terms').val(invoice.invoice_terms); $('#tax_amount').val(parseFloat(invoice.tax_amount).toFixed(2));
                $('#iTableBody').empty();
                items.forEach(item => {
                ...
                calcInvoiceTotals(); invoiceModal.show();
...
function addNewInvoiceRow(data = null) {
    ...
    let costField = (userType === 'Admin') ? `<input type="number" class="table-input table-input-readonly" name="items[${invoiceRowCount}][buy_price]" id="buy-${invoiceRowCount}" value="${buy}" step="0.01" readonly tabindex="-1">` : `<input type="hidden" name="items[${invoiceRowCount}][buy_price]" id="buy-${invoiceRowCount}" value="${buy}">`;
    let costTd = (userType === 'Admin') ? `<td>${costField}</td>` : `${costField}`; 
    let profitTd = (userType === 'Admin') ? `<td><input type="text" class="table-input table-input-readonly text-profit profit-zero" id="profit-${invoiceRowCount}" value="0.00" readonly tabindex="-1"></td>` : ``; 

    let html = `<tr id="row-${invoiceRowCount}"><td class="text-center row-number-cell"><span class="row-number-text">${invoiceRowCount}</span></td><td class="text-center"><div class="item-thumb-box" id="thumb-box-${invoiceRowCount}">${imgHtml}</div></td><td><select class="form-select item-select" id="sel-${invoiceRowCount}" name="items[${invoiceRowCount}][product_id]" onchange="itemSelected(this, ${invoiceRowCount})" style="width:100%" required>${opts}</select><input type="hidden" name="items[${invoiceRowCount}][item_name]" id="name-${invoiceRowCount}" value="${name}"></td><td><div class="input-group input-group-sm"><textarea class="form-control" name="items[${invoiceRowCount}][serial_number]" id="serial-${invoiceRowCount}" rows="1" placeholder="Serial/Warranty" style="resize:none; font-size:0.85rem;">${serial}</textarea><button type="button" class="btn btn-outline-secondary" onclick="openSerialPopup(${invoiceRowCount})" title="Open Serial Editor"><i class="fas fa-list"></i></button></div></td>${costTd}<td><input type="number" class="table-input" name="items[${invoiceRowCount}][unit_price]" id="price-${invoiceRowCount}" value="${price}" step="0.01" oninput="calcInvoiceRow(${invoiceRowCount})" required></td><td><input type="number" class="table-input text-center" name="items[${invoiceRowCount}][quantity]" id="qty-${invoiceRowCount}" value="${qty}" step="any" oninput="calcInvoiceRow(${invoiceRowCount})" required></td>${profitTd}<td><input type="text" class="table-input table-input-readonly fw-bold text-dark" id="total-${invoiceRowCount}" value="0.00" readonly tabindex="-1"></td><td class="text-center"><button type="button" class="btn btn-sm text-danger border-0 bg-transparent" onclick="removeInvoiceRow(${invoiceRowCount})" tabindex="-1"><i class="fas fa-times"></i></button></td></tr>`;

    $('#iTableBody').append(html);
    $(`#sel-${invoiceRowCount}`).select2({ dropdownParent: $('#invoiceModal'), theme: 'bootstrap-5', templateResult: formatProduct, templateSelection: formatProductSelection, width: '100%' });
    if(data) calcInvoiceRow(invoiceRowCount);
    const newRow = document.getElementById(`row-${invoiceRowCount}`); if(newRow) { newRow.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); }
}

function openSerialPopup(id) {
    currentSerialRowId = id;
    let val = $(`#serial-${id}`).val();
    $('#serialModalInput').val(val); $('#serialModalQty').text($(`#qty-${id}`).val());
    let lines = val.split(/\r\n|\r|\n/).filter(line => line.trim() !== "").length; $('#serialLineCount').text(lines);
    serialModal.show();
}

function saveSerialFromModal() { if(currentSerialRowId !== null) { $(`#serial-${currentSerialRowId}`).val($('#serialModalInput').val()); serialModal.hide(); } }

function itemSelected(select, id) { 
    let pid = $(select).val(); let p = productsDB.find(x => x.product_id == pid); if (!p) return; 
    $(`#name-${id}`).val(p.product_name); $(`#price-${id}`).val(parseFloat(p.sell_price).toFixed(2)); $(`#buy-${id}`).val(parseFloat(p.buy_price).toFixed(2)); 
    ...
    $(`#thumb-box-${id}`).html(displayHtml); calcInvoiceRow(id); 
}

function calcInvoiceRow(id) { 
    let qty = parseFloat($(`#qty-${id}`).val()) || 0; let price = parseFloat($(`#price-${id}`).val()) || 0; let buy = parseFloat($(`#buy-${id}`).val()) || 0; 
    let lineTotal = (qty * price).toFixed(2); let lineProfit = ((price - buy) * qty).toFixed(2);
    $(`#total-${id}`).val(lineTotal); 
    if (userType === 'Admin') {
        let profitEl = $(`#profit-${id}`); profitEl.val(lineProfit); profitEl.removeClass('profit-positive profit-negative profit-zero');
        if (lineProfit > 0) profitEl.addClass('profit-positive'); else if (lineProfit < 0) profitEl.addClass('profit-negative'); else profitEl.addClass('profit-zero');
    }
    calcInvoiceTotals(); 
}

function removeInvoiceRow(id) { 
    const tableBody = document.getElementById('iTableBody'); if (tableBody.rows.length <= 1) { Swal.fire('Warning', 'අවම වශයෙන් එක් භාණ්ඩයක්වත් තිබිය යුතුයි.', 'warning'); return; }
    $(`#row-${id}`).remove(); calcInvoiceTotals(); 
}

function calcInvoiceTotals() { 
    let sub = 0; let totProfit = 0;
    $('input[id^="total-"]').each(function() { sub += parseFloat($(this).val()) || 0; }); 
    if (userType === 'Admin') { $('input[id^="profit-"]').each(function() { totProfit += parseFloat($(this).val()) || 0; }); }
    let tax = parseFloat($('#invoiceForm #tax_amount').val()) || 0; let grandTotal = sub + tax;
    $('#invoiceForm #dispSub').text(sub.toFixed(2)); $('#invoiceForm #sub_total').val(sub.toFixed(2)); 
    $('#invoiceForm #dispGrand').text(grandTotal.toFixed(2)); $('#invoiceForm #grand_total').val(grandTotal.toFixed(2)); 
    if (userType === 'Admin') { $('#invoiceForm #dispProfit').text(totProfit.toFixed(2)); }
}

function submitInvoiceForm() { 
    if($('#invoiceForm #client_id').val() == '' || $('input[id^="total-"]').length == 0) { Swal.fire({ title: 'දත්ත මදි!', text: 'කරුණාකර පාරිභෝගිකයෙකු තෝරා භාණ්ඩ ඇතුළත් කරන්න.', icon: 'warning', confirmButtonText: 'හරි' }); return; } 
    ...
    Swal.fire({ title: 'ඉන්වොයිසිය සුරැකීමට අවශ්‍යද?', text: 'ඉන්වොයිසිය සුරැකීමට/යාවත්කාලීන කිරීමට තහවුරු කරන්න.', icon: 'question', showCancelButton: true, confirmButtonText: 'ඔව්, සුරකින්න', cancelButtonText: 'අවලංගු කරන්න' }).then((result) => {
        if (result.isConfirmed) {
            ...
            $.post(actionUrl, $('#invoiceForm').serialize(), function(res) { 
                if(res.status == 'success') { invoiceModal.hide(); Toast.fire({ icon: 'success', title: 'සාර්ථකයි!', text: res.message }).then(() => location.reload()); } 
                else { Swal.fire({ title: 'දෝෂයක්!', text: res.message, icon: 'error', confirmButtonText: 'හරි' }); } 
            }, 'json').fail(function() { Swal.fire({ title: 'තාක්ෂණික දෝෂයක්!', text: "සර්වර් සම්බන්ධතා දෝෂයක් සිදුවිය.", icon: 'error' }); });
        }
    });
}
...
function deleteInvoice(id) {
    actionDrawer.hide();
    Swal.fire({
        title: 'ඉන්වොයිසිය මකන්නද?',
        html: "<p>ඔබට මෙම ඉන්වොයිසිය ස්ථිරවම මැකීමට අවශ්‍යද? <br><strong class='text-danger'>අලෙවි වූ භාණ්ඩ නැවත ස්ටොක් (Stock) වෙත එකතු වනු ඇත.</strong></p>",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
             window.location.href = 'invoices.php?delete_id=' + id;
        }
    });
}
</script>